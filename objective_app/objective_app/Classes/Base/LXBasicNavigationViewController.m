//
//  LXBasicNavigationViewController.m
//  objective_app
//
...
@interface LXBasicNavigationViewController () <UINavigationControllerDelegate>

@end

@implementation LXBasicNavigationViewController
...
- (void)pushViewController:(UIViewController *)viewController animated:(BOOL)animated {
    if (self.viewControllers.count > 0) { //非根控制器才能全屏滑动返回
        ...
        // 设置统一返回的按钮样式
        UIBarButtonItem *backItem = [UIBarButtonItem itemWithImage:@"icon_return"
                                                      highlightImg:@"icon_return"
                                                            target:self
                                                            action:@selector(pop)];
